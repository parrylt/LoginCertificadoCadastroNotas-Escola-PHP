...
<div id="texto" class="shadow-lg p-3 mb-5 bg-white rounded d-inline-flex p-2 bd-highlight">
	...
	<div>
		<label class="col-md-3" for="inputEmail" class="form-label">E-mail: </label>
		<input class="col-md-8" type="email" class="form-control" name="email" id="inputEmail" placeholder="E-mail" required>
	</div>
	<div>
		<label class="col-md-3" for="inputSenha" class="form-label">Senha</label>
		<input class="col-md-8" type="password" class="form-control" name="senha" id="inputSenha" placeholder="Senha" required>
	</div>
		<div>	
			<br><br><br>	
		<button id="btnLoginProfessor" type="submit" class="btn btn-primary" formvalidate>Login</button>
		</div>
	</form>
</div>
    ...
    $con = mysqli_connect ($host, $user, $pass, $base);
        ...
        else
        {
            if ($email != '')
            {
                ...
                $teste = "SELECT * FROM loginProfessor WHERE email = '$email';";
